...
import { sampleWithRequiredData } from '../societe.test-samples';
...
describe('Societe Management Component', () => {
  ...
  it('should call load all on init', () => {
    ...
    expect(comp.societes()[0]).toEqual(expect.objectContaining({ id: 5934 }));
  });

  describe('trackId', () => {
    it('should forward to societeService', () => {
      const entity = { id: 5934 };
      jest.spyOn(service, 'getSocieteIdentifier');
      const id = comp.trackId(entity);
      expect(service.getSocieteIdentifier).toHaveBeenCalledWith(entity);
